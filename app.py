...
app.layout = html.Div(style={'backgroundColor': colors['background'], 'font-family': "Helvetica"}, children=[
    html.H1(
        children='Stock Forecast Prediction',
        style={
            'textAlign': 'center',
            'color': colors['text']
        }
    ...
    html.Div(children='Stock price forecasting web application framework for Python.', style={
        'textAlign': 'center',
        'color': colors['text']
    }),
...
def update_scatter(symbol, n_clicks):
    if n_clicks == 0:
        ...
        df = ticker_data.history(period='1d', start=datetime(2020, 1, 1), end=datetime.now())
    ...
    # Call Prophet() to make prediction up to 200 days
